...
// static
void FlutterTimezonePlusPlugin::RegisterWithRegistrar(
    flutter::PluginRegistrarWindows *registrar) {
  auto channel =
      std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
          registrar->messenger(), "flutter_timezone_plus",
          &flutter::StandardMethodCodec::GetInstance());

  auto plugin = std::make_unique<FlutterTimezonePlusPlugin>();
...
FlutterTimezonePlusPlugin::FlutterTimezonePlusPlugin() {}

FlutterTimezonePlusPlugin::~FlutterTimezonePlusPlugin() {}

void FlutterTimezonePlusPlugin::HandleMethodCall(
    const flutter::MethodCall<flutter::EncodableValue> &method_call,
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (method_call.method_name().compare("getLocalTimezone") == 0)
    {
        std::string standartName = TimeZones::IANA();
        result->Success(flutter::EncodableValue(standartName));
    }
    else if (method_call.method_name().compare("getAvailableTimezones") == 0)
    {
        ...
        flutter::EncodableList list = TimeZones::GetAll();
...
std::string TimeZones::CountryCode() {
    ...
    std::string countryCode = wide_to_utf8(buffer);
    return countryCode;
}
std::string TimeZones::IANA() {
    XmlDocument doc;
    doc.LoadXml(utf8_to_wide(xmlTimezones));
    XmlNodeList mapZones = doc.GetElementsByTagName(L"mapZone");

    std::string standart = Standart();
    std::string countryCode = CountryCode();
    std::string defaultType = "Unknown";
    for (unsigned int i = 0; i < mapZones.Length(); i++) {
        std::string other = wide_to_utf8(mapZones.Item(i).Attributes().GetNamedItem(L"other").InnerText().c_str());
        std::string territory = wide_to_utf8(mapZones.Item(i).Attributes().GetNamedItem(L"territory").InnerText().c_str());
        if (other == standart && territory == countryCode) {
            std::string type = wide_to_utf8(mapZones.Item(i).Attributes().GetNamedItem(L"type").InnerText().c_str());
            return type;
        }
        if (other == standart && territory == "001") {
            defaultType = wide_to_utf8(mapZones.Item(i).Attributes().GetNamedItem(L"type").InnerText().c_str());
        }
    }
    return defaultType;
}
flutter::EncodableList TimeZones::GetAll() {
    XmlDocument doc;
    doc.LoadXml(utf8_to_wide(xmlTimezones));
    XmlNodeList mapZones = doc.GetElementsByTagName(L"mapZone");
    ...
    for (unsigned int i = 0; i < mapZones.Length(); i++) {
        ...
        if (std::find(list.begin(), list.end(), flutter::EncodableValue(type)) == list.end()) {
            list.insert(list.end(), flutter::EncodableValue(type));
        }
    }
    return list;
}
// Source: https://github.com/unicode-org/cldr/blob/main/common/supplemental/windowsZones.xml
std::string TimeZones::xmlTimezones =
